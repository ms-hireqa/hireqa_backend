...
async def jobseeker_signup(
    ...
    try:
        ...
        if hasattr(upload_response, "error") and upload_response.error:
            print("Resume upload error:", upload_response.error)
        ...
        seeker_response = supabase.table("jobseeker_signup").insert([{
            ...
            "resume_uploaded": resume_url,  # ⚠️ Must change column type to text in Supabase
        ...
        if hasattr(seeker_response, "error") and seeker_response.error:
            print("Insert error:", seeker_response.error)
            raise HTTPException(status_code=500, detail={
